...
public class GameField {
    ...
    public synchronized void removeSnakeSilently(Snake snake) {
        snakes.remove(snake);
    }
    public synchronized void removeSnakeWithFoodDrop(Snake snake) {
        Random random = new Random();
        for (SnakesProto.GameState.Coord body : snake.getBody()) {
            if (body.equals(snake.getHead())) continue;
            if (random.nextInt(100) < 50) foods.add(body);
        }
        snakes.remove(snake);
    }
