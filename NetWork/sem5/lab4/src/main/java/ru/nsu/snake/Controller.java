...
public class Controller {
    ...
    public void start() { // receive ANNOUNCEMENT
        serverListListener = new Thread(() -> {
            try {
                ...
                NetworkInterface networkInterface = NetworkInterface.getByInetAddress(InetAddress.getByName(Controller.getLocalIpAddress()));
    ...
    public void startClient(String playerName, ServerInfo serverInfo, boolean isViewer) throws IOException {
        InetAddress address = InetAddress.getByName(serverInfo.serverIPProperty().get());
        int port = serverInfo.serverPortProperty().get();
        SnakesProto.NodeRole role = isViewer ? SnakesProto.NodeRole.VIEWER : SnakesProto.NodeRole.NORMAL;
        snakeNet = new SnakeNet(serverInfo, UI);
        snakeNet.startAsClient(playerName, address, port, role);
    }
