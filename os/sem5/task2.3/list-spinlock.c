...
node_t *create_node(char *val)
{
	...
	node->next = NULL;
	// Для потоков внутри процесса должен быть PRIVATE.
	pthread_spin_init(&node->sync, PTHREAD_PROCESS_PRIVATE);
	// Доступ только между потоками одного процесса
	// (PTHREAD_PROCESS_SHARED требует общей памяти между процессами)
	pthread_spin_init(&node->sync, PTHREAD_PROCESS_PRIVATE);
...
void *print_routine(void *args)
{
	...
	while (!atomic_load(&stop_flag))
	{
		sleep(1);
		printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
			   atomic_load(&inc_iters), atomic_load(&dec_iters), atomic_load(&eq_iters),
