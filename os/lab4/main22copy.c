...
} // как ОС узнает, что стек увеличивается

// почему видим затерты данные после free, если аллокатор по идее просто помечает память свободной
...
int main() {
    ...
    // sleep(5);
    free(allocs);
    // // увидеть аллокацию на куче через strace 
    ...
    //     perror("mprotect");
    // }
    // // почитать в мане почему можно читать хотя прав нет
